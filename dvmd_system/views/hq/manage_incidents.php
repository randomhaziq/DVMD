...
// Base Query - We use 'u.district as district_name' to avoid confusion
...
if ($filter_district) {
    $sql .= " AND u.district = '" . $conn->real_escape_string($filter_district) . "'";
}
if ($filter_severity) {
    $sql .= " AND i.severity = '" . $conn->real_escape_string($filter_severity) . "'";
}
if ($filter_status) {
    $sql .= " AND i.status = '" . $conn->real_escape_string($filter_status) . "'";
}

$sql .= " ORDER BY i.created_at DESC";
$result = $conn->query($sql);

// 4. GET DISTRICT LIST FOR DROPDOWN
$distResult = $conn->query("SELECT DISTINCT district FROM users WHERE district IS NOT NULL AND district != '' ORDER BY district ASC");
...
<div class="incidents-container" style="padding: 20px;">
    ...
    <div style="margin-bottom: 20px;">
        <h2><i class="fas fa-map-marked-alt"></i> National Incident Command</h2>
        <p style="color: #666;">Monitor active disasters and dispatch national resources.</p>
        <?php if (isset($msg))
    ...
    <form method="GET" class="hq-controls">
        ...
        <div class="filter-group">
            ...
            <select name="district">
                ...
                <?php while ($d = $distResult->fetch_assoc()): ?>
                    <option value="<?php echo $d['district']; ?>" <?php echo ($filter_district == $d['district']) ? 'selected' : ''; ?>>
        ...
        <div class="filter-group">
            ...
            <select name="severity">
                ...
                <option value="Critical" <?php if ($filter_severity == 'Critical')
                    echo 'selected'; ?>>Critical</option>
                <option value="High" <?php if ($filter_severity == 'High')
                    echo 'selected'; ?>>High</option>
                <option value="Medium" <?php if ($filter_severity == 'Medium')
                    echo 'selected'; ?>>Medium</option>
                <option value="Low" <?php if ($filter_severity == 'Low')
        ...
        <div class="filter-group">
            ...
            <select name="status">
                ...
                <option value="In Progress" <?php if ($filter_status == 'In Progress')
                    echo 'selected'; ?>>In Progress
                </option>
                <option value="Resolved" <?php if ($filter_status == 'Resolved')
                    echo 'selected'; ?>>Resolved</option>
            </select>
        </div>

        <button type="submit" class="btn-filter">Apply Filters</button>
        <a href="?page=manage_incidents" class="btn-reset">Reset</a>
    </form>
        ...
        <table style="width: 100%; border-collapse: collapse;">
            <thead style="background: #2c3e50; color: white;">
                <tr>
                    ...
                    <th style="padding: 15px; text-align: left;">District (Location)</th>
            ...
            <tbody>
                    ...
                    <?php while ($row = $result->fetch_assoc()):
                        ...
                        <tr style="border-bottom: 1px solid #eee;">
                            ...
                            <td style="padding: 15px;">
                                <strong><?php echo htmlspecialchars($row['district_name'] ?? 'Unknown District'); ?></strong><br>

                                <small style="color: #888; font-family: monospace;">
                                    <i class="fas fa-location-arrow"></i>
                                    <?php echo htmlspecialchars($row['lat'] . ', ' . $row['lng']); ?>
                                </small>
                            </td>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="7" style="padding: 20px; text-align: center;">No incidents found matching your filters.
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
...
<script>
    // 1. Initialize Map (Default view: Malaysia)
            ...
            var markers = []; // Array to store all our pins

    incidents.forEach(inc => {
                let color = inc.severity === 'Critical' ? 'red' : (inc.severity === 'High' ? 'orange' : 'green');
            ...
            .addTo(map)
            .bindPopup(`<b>${inc.type}</b><br>${inc.desc}`);
    ...
    // 3. SMART ZOOM LOGIC
    // If we have pins, zoom to fit them perfectly
    if (markers.length > 0) {
        var group = L.featureGroup(markers); // Group them together
                map.fitBounds(group.getBounds(), {padding: [50, 50] }); // Zoom to fit with 50px padding
    }
                ...
                document.getElementById('modalDesc').innerText = `Incident: ${data.incident_type} @ ${data.district_name || 'Unknown'}`;
    }
<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
<script>
    // 1. Initialize Map
    ...
    // 4. ADD LEGEND (Updated for 4 Colors)
    ...
    legend.onAdd = function (map) {
        ...
        // Add the color items
        div.innerHTML += '<div style="margin-bottom: 5px;"><i style="background:red; width:10px; height:10px; display:inline-block; border-radius:50%; margin-right:5px;"></i> <b>Critical</b></div>';
        div.innerHTML += '<div style="margin-bottom: 5px;"><i style="background:orange; width:10px; height:10px; display:inline-block; border-radius:50%; margin-right:5px;"></i> <b>High</b></div>';
        div.innerHTML += '<div style="margin-bottom: 5px;"><i style="background:gold; width:10px; height:10px; display:inline-block; border-radius:50%; margin-right:5px;"></i> <b>Medium</b></div>';
        div.innerHTML += '<div><i style="background:green; width:10px; height:10px; display:inline-block; border-radius:50%; margin-right:5px;"></i> <b>Low</b></div>';
        
        return div;
    };
    legend.addTo(map);

    // 5. Modal Function (No changes needed here)
    function openDispatch(data) {
        document.getElementById('dispatchModal').style.display = 'flex';
        document.getElementById('modalId').value = data.id;
        document.getElementById('modalStatus').value = data.status;
